...
def parse_excel(file_obj, denomination: str) -> dict[str, dict]:
    """
    Parse an uploaded Excel workbook.

    Expected sheet structure:
      Each sheet represents one company.  Column A = line item labels.
      Remaining columns = fiscal year values (e.g. FY2022, FY2023, FY2024).

    Alternatively, a single-sheet workbook with one company is supported.
    ...
    for sheet_name in xl.sheet_names:
        try:
            df = xl.parse(sheet_name, header=0, index_col=None, dtype=str)
        except Exception:
            continue
        ...
        year_cols  = [c for c in df.columns[1:] if str(c).strip() not in ("", "nan")]
        if not year_cols:
            continue

        company_data: dict = {"years": year_cols, "data": {}, "unmapped": []}
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded = st.file_uploader(
            ...
            type=["xlsx"],
            help=(
                "Each sheet = one company. "
                "Column A = line item labels. "
                "Remaining columns = fiscal years."
            )
        )
        ...
        st.divider()
        st.markdown("### 🎛️ Projection Drivers (FY2025E–FY2027E)")
        st.caption("These assumptions apply uniformly to all uploaded companies.")
        ...
        show_demo = st.checkbox("Use demo data (WING/BROS/CAVA)", value=(uploaded is None))

    # ── Load Data ──────────────────────────────────────────────────────────────
    if uploaded and not show_demo:
        with st.spinner("Parsing workbook and normalizing line items…"):
            companies = parse_excel(uploaded, denomination)
        if not companies:
            st.error("No recognizable company sheets found. Check the input format in the Instructions tab.")
            companies = {}
    else:
        companies = DEMO_COMPANIES

    if not companies:
        st.warning("Upload an Excel workbook or enable demo data to begin.")
        return

    # Compute metrics and projections
    metrics_all: dict[str, dict] = {}
    proj_all:    dict[str, dict] = {}
    proj_labels = ["FY2025E", "FY2026E", "FY2027E"]
    ...
    with tab2:
        ...
        # Projection summary
        st.markdown('<div class="section-hdr">FY2025E–FY2027E Projection Summary</div>',
                    unsafe_allow_html=True)
